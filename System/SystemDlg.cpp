...
//	ON_COMMAND(ID_update, &CAboutDlg::sds)
END_MESSAGE_MAP()
...
void CSystemDlg::OnBnClickedConnect()
{
	...
	else
	{
		...
		sprintf_s(query, "select * from manager where username='%s'and password='%s'",Username,Password);//读取数据库中的数据表manager中的账号密码
		mysql_real_query(&mysqlCon, query, (unsigned long)strlen(query));//执行查询操作
		MYSQL_ROW row;
